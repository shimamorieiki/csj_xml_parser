...
class DaoIPUCore:
    ...
    def parse(self, file_path: str) -> list[DtoSuwCore]:
        """[summary]
        ...
        """
        suw_list: list[DtoSuwCore] = []
        xml_data = self.load_xml(file_path=file_path)
        ...
        for index, ipu in enumerate(root.iter("IPU")):
            ...
            for suw in ipu.iter("SUW"):
                dto_suw: DtoSuwCore = DtoSuwCore()
                dto_suw.suw_id = suw.get("SUWID", "")
                dto_suw.column_id = int(suw.get("ColumnID", -1))
                ...
                dto_suw.clause_unit_id = int(suw.get("ClauseUnitID", -1))
                ...
                dto_suw.dep_bunsetu_unit_id = int(suw.get("Dep_BunsetsuUnitID", -1))
                dto_suw.dep_modifiee_bunsetu_unit_id = int(
                    suw.get("Dep_ModifieeBunsetsuUnitID", -1)
                )
    ...
    @staticmethod
    def return_sentence_directly(dto_suws: list[DtoSuwCore]) -> list[str]:
        """[summary]
        Args:
